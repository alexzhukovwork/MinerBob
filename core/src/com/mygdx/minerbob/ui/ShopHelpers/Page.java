...
public class Page {
    ...
    public Page(GameWorld gameWorld, int amount, int number) {
        ...
        int index = 0;
        Item item;
        float tx = gameWorld.WIDTH / 14f, ty = gameWorld.HEIGHT / 7.5f;
        for(int i = 0; i < amountRow; i++) {
            for (int j = 0; j < amountCol && items.size < amount; j++) {
                index = number * 6 + j + 2 * (i);
                item = new Item(gameWorld, index, tx + gameWorld.WIDTH * number, ty,
                        gameWorld.WIDTH / 2.5f, gameWorld.HEIGHT / 4.5f, gameWorld.assetLoader.textures.get(index).cost,
                ...
                items.add(item);
                tx += gameWorld.WIDTH / 2.5f + 5;
            }
            ty += gameWorld.HEIGHT / 4.5f + 5;
            tx = gameWorld.WIDTH / 14f;
        }
    }
